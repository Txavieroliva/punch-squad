...
func _physics_process(delta: float) -> void:
	...
	# Knockback corregido (solo horizontal, decay suave)
	if player.knockback_velocity != Vector2.ZERO:
		player.velocity += player.knockback_velocity * delta
		player.knockback_velocity = player.knockback_velocity.lerp(Vector2.ZERO, 0.2)  # Decay suave
		if player.knockback_velocity.length() < 5:
